...
export const TicketSelection = ({ setIsTicketSelected, showErrors }) => {
  ...
  useEffect(() => {
    const savedTicketDetails = localStorage.getItem("Details");
    ...
    // Initialize
    Details.ticketTypes.forEach((type) => {
 ...
 useEffect(() => {
  ...
  const updateLocalStorage = (tickets, ticketType) => {
    ...
    localStorage.setItem("Details", JSON.stringify(ticketDetails));
  };
      ...
      <div className="font-roboto">
        ...
        <div className="p-4 rounded-3xl border border-borderthree bg-greenfive">
          <div className="flex flex-wrap gap-4">
            {Details.ticketTypes.map((ticketType) => {
              ...
              return (
                <div
                  key={ticketType.id}
                  className={`flex flex-col justify-between w-64 h-24 p-3 items-start p-4 rounded-xl ${
                    isDisabled
                      ? "bg-gray-500 border-gray-600 cursor-not-allowed opacity-50"
                      : selectedTicketType === ticketType.type
                      ? "bg-greenone border"
                      : "hover:bg-borderone"
                  } border-2 border-bordertwo w-full transition-all duration-300 ease-in-out cursor-pointer group`}
                  onClick={() =>
                ...
                >
                  <div className="flex flex-col  gap-1 text-lightgrey group-hover:text-white">
                    <div className="flex justify-between items-center gap-4">
                      <h4 className="uppercase text-lightgrey">
                        {ticketType.type}
                      </h4>
                      <p className="font-semibold text-2xl text-lightgrey bg-greensix px-4 py-1 rounded-lg">
                        {ticketType.price === 0
                          ? "Free"
                          : `$${ticketType.price}`}
                      </p>
                    ...
                    <p className="text-sm text-[#D9D9D9]">
                      {ticketsRemaining}/{ticketType.limit}
                    </p>
                  </div>
                </div>
              );
